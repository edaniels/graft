...
Files moved by `graft sync` are written by mutagen under the remote user's account. Graft does not attempt to preserve or translate ownership across machines, and it does not run as root on either end.

Mutagen's synchronization model tracks file content and the POSIX executable bit, not permission bits, so per-file source modes are never mirrored. Instead, every file and directory the sync writes gets a fixed mode from the session configuration. Graft sets these on the beta (remote) endpoint only, in `EstablishSynchronization` in `pkg/connection.go` via `syncBetaConfiguration` in `pkg/synchronization.go`: files land `0644` and directories `0755` on the remote, with the source's executable bit layered on top (so locally-executable files land `0755`). The sync root directory itself is created by graft rather than mutagen; roots graft creates are chmodded to the same directory mode so a restrictive remote umask cannot produce an untraversable root, while pre-existing roots are left untouched (they may deliberately carry other modes, such as a `0700` home directory, or be owned by another account). World-readable is deliberate: the usual consumers of a synced tree are remote processes running as other users, most commonly non-root container users reading docker bind mounts, and mutagen's own `0600`/`0700` defaults make the tree unreadable to them.

Two boundaries limit what that exposes. First, the loosening is remote-only: the local endpoint keeps mutagen's `0600`/`0700` defaults, so files the sync creates on the local machine (content that originated on the remote) stay private locally, and permission bits never propagate from the remote to the local machine beyond the executable bit. A compromised remote cannot widen local file modes through the sync. Second, the `.git` replica session created for `syncGit` keeps `0600`/`0700` on the remote (including its graft-created root, pinned to `0700`) unless explicit per-sync modes are configured, which apply to the replica as well: `.git` contents are only needed by the remote user's own git commands, and `.git/config` can embed credentials while `.git/objects` holds full history, so by default they stay readable by the owning account alone.

The remaining exposure is inherent to the model: any locally-private (`0600`) file inside a synced tree becomes readable on the remote by anything that can traverse to it, including every local account on hosts where the path to the sync root is world-traversable (`0700` home directories gate this on RHEL-family remotes; other distributions and sync roots outside `$HOME` may not). Files matching the sync root's top-level `.gitignore` are never synced at all. Where a tree warrants tighter modes, `defaultFileMode` and `defaultDirectoryMode` can be set per synchronization (octal strings, e.g. `"600"`/`"700"`) in the daemon config or a `graft.yaml` destination; they apply to both the working tree and the `.git` replica, and file modes carrying executability bits are rejected at validation since mutagen owns the executable bit. Mode configuration only affects content the sync writes after it applies: mutagen assigns modes as it creates and updates files, so trees synced under an earlier configuration keep their existing modes until each file is next rewritten (a one-time `chmod` on the remote is the practical fix for an already-synced tree).
