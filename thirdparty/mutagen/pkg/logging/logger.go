...
func NewLoggerOnSlogger(logger *slog.Logger) *Logger {
	return &Logger{
		slogger: logger,
	}
}
...
func (l *Logger) Sublogger(name string) *Logger {
	...
	if l.slogger != nil {
		return &Logger{slogger: l.slogger.With("scope", scope)}
	}
...
func (l *Logger) log(level Level, v ...any) {
	if l != nil && l.slogger != nil {
		...
		logRec := slog.NewRecord(time.Now(), toSlogLevel(level), fmt.Sprint(v...), pcs[0])
		l.slogger.Handler().Handle(context.Background(), logRec)
		return
	}
...
func (l *Logger) logf(level Level, format string, v ...any) {
	if l != nil && l.slogger != nil {
		l.slogger.Log(context.Background(), toSlogLevel(level), fmt.Sprintf(format, v...))
		return
	}
	if l != nil && l.level >= level {
